...
suppose that $W = (w_1^T,w_2^T,\cdots,w_k^T)^T$, $b =
  (b_1,b_2,\cdots,b_k)^T$. So we have $$\begin{cases} 
    w_i x+ b_i > w_j x + b_j & x\in A_i \    w_i x+ b_i < w_j x + b_j& x\in A_j \    \end{cases}$$ Take $w_{i,j} = w_i - w_j, b_{i,j} = b_i-b_j$, then we
have $$w_{i,j}x + b_{i,j}\begin{cases} 
    > 0 & x\in A_i \    < 0 & x\in A_j \    \end{cases}$$ So $A_1,...,A_k$ are pairwise linearly separable. ◻
```
